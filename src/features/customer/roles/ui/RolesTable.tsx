...
export function RolesTable({
    ...
    const canView = false//useAccess({ permission: PERMISSIONS.PERMISSION_CUSTOMER_ROLES_VIEW }, true)
    const canEdit = useAccess({ permission: PERMISSIONS.PERMISSION_CUSTOMER_ROLES_EDIT }, true)
    const canDelete = useAccess({ permission: PERMISSIONS.PERMISSION_CUSTOMER_ROLES_DELETE }, true)

    const handleView = useCallback((item: TRole) => {
        navigate(PAGE_URLS.customerRoles.view(item.id))
    }, [navigate]);

    const handleEdit = useCallback((item: TRole) => {
        navigate(PAGE_URLS.customerRoles.update(item.id))
    }, [navigate]);
