...
export function GroupsTable({
    ...
    const canView = false//useAccess({ permission: PERMISSIONS.PERMISSION_CUSTOMER_GROUPS_VIEW }, true)
    const canEdit = useAccess({ permission: PERMISSIONS.PERMISSION_CUSTOMER_GROUPS_EDIT }, true)
    const canDelete = useAccess({ permission: PERMISSIONS.PERMISSION_CUSTOMER_GROUPS_DELETE }, true)

    const handleView = useCallback((item: TGroup) => {
        navigate(PAGE_URLS.customerGroups.view(item.id))
    }, [navigate]);

    const handleEdit = useCallback((item: TGroup) => {
        navigate(PAGE_URLS.customerGroups.update(item.id))
    }, [navigate]);
